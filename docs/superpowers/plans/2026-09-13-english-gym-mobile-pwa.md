...
Expected: шесть файлов — `pwa-64x64.png`, `pwa-192x192.png`, `pwa-512x512.png`, `maskable-icon-512x512.png`, `apple-touch-icon-180x180.png`, `favicon.ico`. Каждый весит больше нуля байт. Пустой или однобайтовый файл означает, что растеризация провалилась; настоящая проверка — не размер, а осмотр глазами на следующем шаге.
